...
def detect_project_type(project_path: Path) -> dict:
    ...
    if package_json.exists():
        ...
        try:
            ...
            if "lint" in scripts:
                cmd_name = "npm.cmd" if os.name == 'nt' else "npm"
                result["linters"].append({"name": "npm lint", "cmd": [cmd_name, "run", "lint"]})
            elif "eslint" in deps:
                cmd_name = "npx.cmd" if os.name == 'nt' else "npx"
                result["linters"].append({"name": "eslint", "cmd": [cmd_name, "eslint", "."]})
            
            # Check for TypeScript
            if "typescript" in deps or (project_path / "tsconfig.json").exists():
                cmd_name = "npx.cmd" if os.name == 'nt' else "npx"
                result["linters"].append({"name": "tsc", "cmd": [cmd_name, "tsc", "--noEmit"]})
