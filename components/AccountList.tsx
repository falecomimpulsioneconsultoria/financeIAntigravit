  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {viewMode === 'GRID' ? (
              ...
              <div key={acc.id} className="group relative bg-white rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 overflow-hidden">
                ...
                <div className="p-6 relative z-10">
                  ...
                  <div className="mt-2 pt-2 border-t border-gray-50 flex items-center justify-between">
                    <div className="flex flex-col"><span className="text-sm font-medium text-gray-700">{acc.name}</span><span className="text-[10px] text-gray-400 font-bold uppercase">{acc.type === 'CASH' ? 'Dinheiro' : acc.type === 'INVESTMENT' ? 'Investimento' : 'Corrente'}</span></div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col pt-2">
          {/* List View */}
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="text-[10px] text-gray-400 uppercase tracking-widest bg-gray-50/50 border-b border-gray-100">
                <tr>
                  <th className="px-6 py-4 font-bold">Conta</th>
                  <th className="px-6 py-4 font-bold">Tipo</th>
                  <th className="px-6 py-4 font-bold text-right">Saldo Atual</th>
                  <th className="px-6 py-4 font-bold text-right">Projeção</th>
                  {canManage && <th className="px-6 py-4 font-bold text-right">Ações</th>}
                </tr>
              </thead>
                    ...
                    <tr key={acc.id} className="hover:bg-gray-50/50 transition-colors group">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${cfg.light} ${cfg.text}`}>
                            {getTypeIcon(acc.type || 'CHECKING')}
                          </div>
                          <span className="font-bold text-gray-700">{acc.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-500 font-medium">
                        {acc.type === 'CASH' ? 'Dinheiro / Carteira' : acc.type === 'INVESTMENT' ? 'Investimento / Corretora' : 'Conta Corrente / Banco'}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <span className={`font-bold ${acc.projectedBalance >= acc.balance ? 'text-blue-600' : 'text-orange-500'}`}>
                          R$ {acc.projectedBalance.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                      </td>
                      {canManage && (
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <div className="inline-flex opacity-0 group-hover:opacity-100 transition-opacity gap-2">
                            <button onClick={() => handleEditClick(acc)} className="p-1.5 text-blue-600 bg-white border border-gray-100 hover:bg-blue-50 rounded-lg shadow-sm" title="Editar">
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                            </button>
                            <button onClick={() => handleDeleteClick(acc.id, acc.name)} className="p-1.5 text-rose-600 bg-white border border-gray-100 hover:bg-rose-50 rounded-lg shadow-sm" title="Excluir">
          ...
          {canManage && (
            <div className="p-4 bg-gray-50/50 border-t border-gray-100 text-center">
              <button onClick={() => setIsModalOpen(true)} className="inline-flex items-center gap-2 text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors px-4 py-2">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-scale-in">
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Saldo Atual</label><div className="relative"><span className="absolute left-4 top-2.5 text-gray-500 font-medium">R$</span><NumericFormat required value={formData.balance === 0 ? '' : formData.balance} onValueChange={(values) => setFormData({ ...formData, balance: values.floatValue || 0 })} thousandSeparator="." decimalSeparator="," decimalScale={2} fixedDecimalScale allowNegative={true} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" /></div></div>
