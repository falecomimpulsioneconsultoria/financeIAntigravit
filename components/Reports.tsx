...
export const Reports: React.FC<ReportsProps> = ({ transactions, categories, user }) => {
    ...
    const [filterStart, setFilterStart] = useState(() => {
        const d = new Date(selectedDate); return new Date(d.getFullYear(), d.getMonth(), 1).toISOString().split('T')[0];
    });
    const [filterEnd, setFilterEnd] = useState(() => {
        const d = new Date(selectedDate); return new Date(d.getFullYear(), d.getMonth() + 1, 0).toISOString().split('T')[0];
    });

    const handleMonthChange = (newDate: Date) => {
        setSelectedDate(newDate);
        setFilterStart(new Date(newDate.getFullYear(), newDate.getMonth(), 1).toISOString().split('T')[0]);
        setFilterEnd(new Date(newDate.getFullYear(), newDate.getMonth() + 1, 0).toISOString().split('T')[0]);
        setAiAnalysis(null);
    };
    ...
    const activeTransactions = useMemo(() => {
        ...
        if (effectiveBasis === 'CASH') {
            return transactions.filter(t => {
                if (t.status !== 'PAID' || !t.paymentDate || t.paymentDate < filterStart || t.paymentDate > filterEnd) return false;
        ...
        return transactions.filter(t => {
            if (t.date < filterStart || t.date > filterEnd) return false;
            if (t.parentId) return false;
            return true;
        });
    }, [transactions, filterStart, filterEnd, accountingBasis, activeTab, childrenMap]);
    ...
    const kpis = useMemo(() => {
        ...
        if (todayStr >= filterStart && todayStr <= filterEnd) {
            const timePassed = Math.abs(today.getTime() - start.getTime());
            daysPassed = Math.ceil(timePassed / (1000 * 60 * 60 * 24));
        } else if (todayStr < filterStart) {
            daysPassed = 1;
        }
    ...
    }, [activeTransactions, filterStart, filterEnd]);
