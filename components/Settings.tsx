...
import { DashboardConfig, User, SubUser, UserPermissions, PaymentMethod, Transaction, DescriptionSuggestion } from '../types';
...
interface SettingsProps {
    ...
    onDeleteDescriptionSuggestion?: (id: string) => void;
}

type TabType = 'PROFILE' | 'SECURITY' | 'USERS' | 'PAYMENT_METHODS' | 'PREFERENCES' | 'SUGGESTIONS';
...
export const Settings: React.FC<SettingsProps> = ({
    ...
    descriptionSuggestions = [], onDeleteDescriptionSuggestion
}) => {
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                <div className="w-full lg:w-64 flex-shrink-0 space-y-2">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
                        ...
                        <TabButton id="SUGGESTIONS" label="Sugestões" icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" /></svg>} />
                    </div>
                </div>
                ...
                <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                                ...
                                {subUsers.map(u => (
                                    <div key={u.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white border rounded-xl shadow-sm gap-4">
                                        ...
                                        <div className="flex gap-1 self-end sm:self-center">
                                            <button onClick={() => handleEditSubUser(u)} className="p-2 text-blue-400 hover:bg-blue-50 rounded-lg transition-colors"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                                            <button onClick={() => handleRemoveSubUser(u.id)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg transition-colors"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    {activeTab === 'SUGGESTIONS' && (
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 animate-scale-in">
                            ...
                            <p className="text-gray-500 mb-6 text-sm">Gerencie as descrições salvas que aparecem como preenchimento automático no cadastro de lançamentos.</p>
                            ...
                            {descriptionSuggestions.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                    {descriptionSuggestions.map((sug) => (
                                        <div key={sug.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50/30 transition-colors">
                                            <span className="font-medium text-gray-700">{sug.text}</span>
                                            <button 
                                                onClick={() => onDeleteDescriptionSuggestion && onDeleteDescriptionSuggestion(sug.id)}
