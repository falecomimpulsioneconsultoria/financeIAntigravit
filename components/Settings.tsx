  ...
  const TabButton = ({ id, label, icon }: { id: TabType, label: string, icon: React.ReactNode }) => (
      <button 
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
            activeTab === id ? 'bg-blue-50 text-blue-700 shadow-sm ring-1 ring-blue-100' : 'text-gray-600 hover:bg-gray-50'
        }`}
      >
        {icon}
        {label}
              ...
              {activeTab === 'USERS' && (
                  <div className="space-y-6 animate-fade-in">
                      <div className="space-y-4">
                          {subUsers.map(u => (
                              <div key={u.id} className="flex items-center justify-between p-4 bg-white border rounded-xl shadow-sm">
                                  <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold">{u.name.charAt(0)}</div><div><p className="font-bold">{u.name}</p><p className="text-xs text-gray-400">{u.email}</p></div></div>
                                  <div className="flex gap-1"><button onClick={() => handleEditSubUser(u)} className="p-2 text-blue-400 hover:bg-blue-50 rounded-lg"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5" strokeWidth={2} /></svg></button><button onClick={() => handleRemoveSubUser(u.id)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7" strokeWidth={2} /></svg></button></div>
                              </div>
                          ))}
                      </div>
                      <div className="p-6 bg-gray-50 rounded-xl border space-y-4">
                          <h5 className="text-sm font-bold uppercase">{editingSubUserId ? 'Editar Usuário' : 'Novo Usuário'}</h5>
                          <div className="grid grid-cols-2 gap-4"><input placeholder="Nome" value={newSubUserName} onChange={e => setNewSubUserName(e.target.value)} className="px-4 py-2 bg-white border rounded-lg" /><input placeholder="Email" value={newSubUserEmail} onChange={e => setNewSubUserEmail(e.target.value)} className="px-4 py-2 bg-white border rounded-lg" /></div>
                          <input type="password" placeholder="Senha" value={newSubUserPassword} onChange={e => setNewSubUserPassword(e.target.value)} className="w-full px-4 py-2 bg-white border rounded-lg" />
                          <div className="flex justify-end gap-2">{editingSubUserId && <Button variant="ghost" onClick={handleCancelEdit}>Cancelar</Button>}<Button onClick={handleSaveSubUser}>Salvar</Button></div>
                      </div>
                  </div>
              )}
